 ...
 * 模块职责：Run 清理服务。
 *
 * 处理"运行到一半服务没了"的补偿场景。进程重启后，Redis 里的运行中草稿/锁可能丢失，
 * 但 SQLite 里还会留下 status=running 的 run/message/tool/resource/process_step。
 * 这个类在服务启动时把这些悬挂记录统一标成 failed，并追加可回放事件，让前端和审计视角都闭环。
 *
 * 边界：
 * - 只处理"服务重启/进程退出导致的悬挂运行"，不处理正常用户取消（那走 cancelRun）。
 * - 只补偿 status=running 的记录，历史成功/失败记录不受影响。
 * - 清理 Redis 草稿是 best-effort，Redis 不可用时 SQLite 失败态已经写完。
 */
...
export class AgentRunCleanupService {
  ...
  /**
   * 清理所有不在 activeRunIds 中的 running run。
   * activeRunIds 是当前进程内还握着 AbortController 的 run，不在这里面的 running run
   * 基本就是上个进程遗留下来的，需要补偿成失败态。
   * 返回清理统计（runs/messages/toolCalls/resources/processSteps 各清理了多少条）。
   */
