/**
 * 模块职责：消息协调器——Agent run 生命周期的总编排。
 *
 * 这个文件是 Agent 系统的"中枢神经"，负责把一次用户输入编排成完整的执行链路：
 *   创建 message/run → 投递队列（或本地执行）→ Worker 执行 → 事件投影 → 最终落库。
 *
 * 核心设计原则：
 * 1. 高频运行态（answer_delta、running parts）写 Redis 草稿，最终结果才写 SQLite。
 *    这样 SQLite 保存的是可审计的稳定结果，Redis 只承担生成过程中的临时状态。
 * 2. 事件分两类：持久事件（appendRunEvent）写库 + fanout；临时事件（publishTransientEvent）
 *    只走 Redis Pub/Sub 不写库（如高频 delta）。断线恢复靠 message snapshot，排查靠 OTel logs。
 * 3. coordinator 只管"主流程编排"（建 run、排队、取消、最终落库），把草稿管理、进度步骤、
 *    资源投影、重启清理分别委托给独立小类，避免这个文件重新膨胀。
 *
 * 边界：
 * - 不直接调用 LLM，而是委托给 AgentRunner（agentService）。
 * - 不直接操作 S3，而是委托给 ToolResourceStorage。
 * - 不直接做上下文压缩，而是委托给 AgentSummaryService。
 */
...
/**
 * 消息协调器。管理 Agent 会话、消息、run 的完整生命周期。
 *
 * 这个类同时承担"API 层入口"和"Worker 执行入口"两个角色：
 * - API 层调用 startRun / cancelRun / getSession 等，快速返回 runId 给前端；
 * - Worker 进程调用 executeQueuedRun，真正跑模型执行循环。
 *
 * 内部委托了四个小类来避免自身膨胀：
 * - draftManager：管理运行中草稿（Redis 里的 parts/delta）；
 * - processStepProjector：把流式事件翻译成用户可读的进度步骤；
 * - resourceOutputProjector：把工具产出的资源投影到 resource 表和 message parts；
 * - cleanupService：服务重启时清理悬挂的 running run。
 */
export class AgentMessageCoordinator {
  ...
  /**
   * 启动一次 Agent run。这是 API 请求的入口方法。
   * 职责：创建 user message + run 记录 → 如果有队列就投递 job → 返回 runId 给前端。
   * 不在请求线程里跑模型，而是交给 Worker 异步执行。前端拿到 runId 后通过 SSE 订阅执行进度。
   */
  ...
  /**
   * 重新生成指定助手消息。用户对当前回答不满意时调用。
   * 会找到原回答对应的用户输入，重建对话历史，并重放资源类工具调用（避免重复生成图片）。
   * 不走队列，直接在当前进程同步执行（适用于测试或无队列部署）。
   */
  ...
  /**
   * 创建 run 执行守卫。在执行循环中定期检查 run 是否还活着（未被取消、未崩溃）。
   * 做了 500ms 节流：delta 事件非常高频，每次都查 SQLite 会拖慢生成速度。
   * 关键事件（tool_start/tool_result 等）强制检查，避免取消后还继续落库工具结果。
   */
  ...
  /**
   * 处理 AgentService 发来的执行层事件，把它们分发到正确的投影器。
   * 事件路由优先级：资源类工具事件先交给 resourceOutputProjector，它返回 false 时
   * 才走普通工具调用逻辑。所有事件最终都经过 processStepProjector 投影成进度步骤，
   * 并通过 appendEvent 推给 SSE。
   */
  ...
  /**
   * 在正式回答前刷新会话摘要（上下文压缩）。
   * 触发条件：上一份摘要之后新增的消息数超过阈值，且内容字符数足够（避免对短消息频繁压缩）。
   * 压缩产出 system message + summary 事件，挂在当前 run 上。
   * 压缩失败不中断主回答流程，只标记 system message 失败并继续。
   */
