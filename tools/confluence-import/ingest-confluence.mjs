...
import { parseEnvFile } from "../dev/ensure-redis.mjs";
...
const HELP = `用法：pnpm confluence:ingest [选项]
...
async function listMode(config, confluence) {
  ...
  console.log(`\n确认无误后执行：pnpm confluence:ingest --ingest${config.section ? ` --section "${config.section}"` : ""}`);
}
