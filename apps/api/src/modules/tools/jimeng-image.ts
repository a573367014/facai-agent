/**
 * 即梦图片生成工具（火山引擎 Seedream / SeedEdit）
 *
 * 提供两个工具：
 * - generate_image：文生图（支持单张和最多 5 张批量），走火山 CVSync2AsyncSubmitTask 异步任务。
 * - edit_image：基于原图 + 编辑指令的图生图，走 SeedEdit3.0。
 *
 * 两个工具共用一套底层能力：HMAC-SHA256 签名、submit + poll 异步任务模型、
 * 本地图转 base64、以及统一的 llmContent 渲染（只告诉模型"图已生成"，不泄露 URL / base64）。
 *
 * 边界：本文件只对接火山视觉接口；超时、权限、可观测性等通用治理仍由 ToolExecutor 兜底。
 */
 ...
 * 校验图片宽高参数。
 *
 * 规则：width 和 height 必须同时出现或同时缺省（避免只改一边导致比例失真）；
 * 且 width * height 不能超过 2048*2048（火山单图像素上限）。
 * 不在校验阶段拦截的话，上游会直接报错，而且错误信息对 LLM 不友好、难以自我纠正。
 */
...
/**
 * 构造文生图工具（generate_image）。
 *
 * 返回的 RegisteredTool 内部封装了火山异步任务的两步流程（提交 → 轮询），
 * 并在批量模式下用有限并发（默认 2）跑多个子任务，通过 emitProgress 上报每个子任务进度。
 * timeoutMs 默认 300s：单张生图 + 轮询通常 10~30s，批量更久，需要给足余量。
 */
...
/**
 * 构造图片编辑工具（edit_image），基于 SeedEdit3.0。
 *
 * 与文生图共用异步任务流程，区别在于提交 body 需要带原图：
 * 本地 /uploads 图片先转 base64 再提交（火山无法访问 localhost），
 * 公网图则直接传 image_urls 让火山自行拉取。
 */
