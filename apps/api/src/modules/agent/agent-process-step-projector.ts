 ...
 * 模块职责：过程步骤投影器。
 *
 * 把底层流式事件（agent_state / llm_response / tool_start / tool_result / tool_error）
 * 翻译成"用户能看懂的任务进度"步骤（thinking / tool / summary），写入 agent_process_steps 表。
 *
 * AgentService 发的是技术事件（iteration/llm/tool），前端聊天区更需要稳定的步骤视图：
 * 理解需求 → 调用工具 → 整理回答。这个类就是两种视图之间的翻译层。
 *
 * 边界：
 * - 只写 process_steps 表和 process.step 事件，不写 tool_call / resource / message part。
 * - 完整工具结果由 tool_call/resource 保存，step 只存摘要，避免进度列表被大段返回污染。
 * - knowledge_search 等内部工具对用户隐藏，不生成进度步骤。
 */
...
export class AgentProcessStepProjector {
  ...
  /**
   * 把一条流式事件投影成 process step 的创建或更新。
   * 每种事件类型有对应的处理逻辑：
   * - thinking：创建"正在理解需求"步骤（只创建一次）；
   * - llm_response：把 thinking 步骤标记为完成（有工具调用→"已理解需求"，无→"已生成回答"）；
   * - answering：只在执行过工具时才创建"正在整理回答"步骤；
   * - tool_start/tool_result/tool_error：创建/更新工具步骤；
   * - error/cancelled：调用 completeRunning 收尾所有 running 步骤。
   */
  ...
  /**
   * 收尾所有仍在 running 的步骤。在 run 成功、失败、取消时调用。
   * 不调用会导致前端出现消息已结束但进度条里还有"正在..."的悬挂状态。
   * "整理回答"等步骤在结束时换成更自然的文案（如"已整理回答"）。
   */
