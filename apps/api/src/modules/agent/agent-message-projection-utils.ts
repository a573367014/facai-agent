...
/**
 * 在事件发布前脱敏。文档工具结果里可能包含 contentBase64 和本地文件路径，
 * 这些信息不应通过 SSE 推给前端。这里把文档结果的 source 替换成 "consumed"，
 * 移除 contentBase64，只保留安全元数据。
 */
...
/**
 * 类型守卫：判断事件是否是"带 toolCallId 的图片工具结果"。
 * coordinator 用它决定是否交给 resourceOutputProjector 处理图片结果。
 */
...
/**
 * 类型守卫：判断事件是否是"带 toolCallId 的视频工具结果"。
 */
...
/**
 * 类型守卫：判断事件是否是"带 toolCallId 的文档工具结果"。
 */
