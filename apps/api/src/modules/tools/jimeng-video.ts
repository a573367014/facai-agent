/**
 * 即梦视频生成工具（火山引擎即梦 AI - 视频生成 3.0 720P）
 *
 * 一个 generate_video 工具覆盖三种上游能力：纯文生视频、首帧图生视频、首尾帧过渡视频，
 * 根据 firstFrameImageUrl / lastFrameImageUrl 是否存在自动选择对应 reqKey。
 *
 * 与图片工具同构：HMAC-SHA256 签名 + submit / poll 异步任务模型 + 本地图转 base64。
 * timeoutMs 默认 600s（10 分钟），因为视频生成远比图片慢，轮询间隔也放大到 5s。
 *
...
/**
 * 把首帧 / 尾帧图片转换成火山提交 body 需要的输入格式。
 *
 * 策略：只有当"所有"帧都能成功转成 base64 时，才走 binary_data_base64；
 * 否则整体回退到 image_urls。混用两种格式火山接口不接受，所以这里用 every 做"全有或全无"的决策。
 */
...
/**
 * 构造视频生成工具（generate_video）。
 *
 * 内部按 selectVideoMode 选择三种模式之一（文生 / 首帧 / 首尾帧），
 * 再走火山的 submit + poll 异步流程拿最终 video_url。
 * timeoutMs 默认 600s，轮询间隔 5s、最多 72 次，覆盖大多数视频生成耗时。
 */
