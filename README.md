...
7. Worker 调用 LLM 和工具；文本 delta 写 Redis running draft，关键事件写本地 JSONL 日志，同时通过 Redis Pub/Sub 推给 API。
...
10. API SSE 收到终态事件后结束连接；刷新页面时用 Redis/SQLite snapshot 恢复视图，不做历史事件全量回放。
...
SQLite 仍然是最终可信的数据源：session、message、run、tool calls、resources、process steps 都在 SQLite；Agent 过程排查看本地 JSONL 日志；Redis 里的内容都可以过期或丢失，最多影响运行中的实时体验，不应该成为最终审计依据。
...
- [x] 移除 event 表，Agent 过程事件改写本地 JSONL 日志。
...
注意：当前产品方向不做完整 delta 回放。实时恢复依赖 `message.snapshot` + running draft；过程事件只做 live 观测和本地日志排查，不作为用户界面恢复源。
